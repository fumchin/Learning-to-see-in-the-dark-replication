...
class FujiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     target_rgb = self.transform(np.uint8(target_rgb))

        # src_img = np.asarray(np.float32(src_img))
        # target_rgb = np.asarray(np.float32(target_rgb))
        ...
        src_pack = torch.mul(src_pack, cfg.amp_ratio)

        # target_img = target_img[0:cfg.patch_size, 0:cfg.patch_size]
        # src_img_save = Image.fromarray((target_rgb*255).astype(np.uint8))
        # src_img_save.save(os.path.join("./src.png"))
...
class FujiDataset_eval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        src_rgb = src_img.postprocess(use_camera_wb=True, half_size=False, no_auto_bright=True, output_bps=16)
        src_rgb = src_rgb.astype(np.float32)
        ...
        # src_rgb = src_rgb[start_H:start_H+cfg.patch_size, start_W:start_W+cfg.patch_size]
        src_rgb = torch.tensor(src_rgb)
        src_rgb = src_rgb.permute(2, 0, 1)

        src_pack = src_pack.permute(2, 0, 1)

        

        return src_pack, target_rgb, src_rgb
    ...
    def pack(self, im):
        ...
        # out[1::2, 0::2, 0] = im[3:H:6, 0:W:6]
        # out[1::2, 1::2, 0] = im[3:H:6, 3:W:6]

        # out[:, :, 1] = im[1:H:3, 1:W:3]
        # out[:, :, 2] = im[1:H:3, 2:W:3]
        # out[:, :, 3] = im[2:H:3, 1:W:3]
        # out[:, :, 4] = im[2:H:3, 2:W:3]
        # # B
        # out[0::2, 0::2, 5] = im[0:H:6, 1:W:6]
        # out[0::2, 1::2, 5] = im[0:H:6, 5:W:6]
        # out[1::2, 0::2, 5] = im[2:H:6, 0:W:6]
        # out[1::2, 1::2, 5] = im[1:H:6, 3:W:6]
        # # B
        # out[0::2, 0::2, 6] = im[3:H:6, 2:W:6]
        # out[0::2, 1::2, 6] = im[3:H:6, 4:W:6]
        # out[1::2, 0::2, 6] = im[4:H:6, 0:W:6]
        # out[1::2, 1::2, 6] = im[5:H:6, 3:W:6]

        # # R
        # out[0::2, 0::2, 7] = im[0:H:6, 2:W:6]
        # out[0::2, 1::2, 7] = im[0:H:6, 4:W:6]
        # out[1::2, 0::2, 7] = im[1:H:6, 0:W:6]
        # out[1::2, 1::2, 7] = im[2:H:6, 3:W:6]
        # # R
        # out[0::2, 0::2, 8] = im[3:H:6, 1:W:6]
        # out[0::2, 1::2, 8] = im[3:H:6, 5:W:6]
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(fuji, batch_size=6, shuffle=False)
